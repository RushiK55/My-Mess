...
class OwnerMessMealsFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.btnShowForm.setOnClickListener {
            ...
            clearForm()
            binding.tvFormTitle.text = "Add New Meal"
        ...
        binding.btnAdd.setOnClickListener {
            ...
            if (selectedImageUri == null && existingImageUrl == null) {
                Toast.makeText(requireContext(), "Please select a meal image", Toast.LENGTH_SHORT).show()
                return@setOnClickListener
            }
    ...
    private fun showMealDetails(meal: Meal) {
        ...
        sheetBinding.btnEdit.setOnClickListener {
            ...
            binding.tvFormTitle.text = "Edit Meal"
                ...
                launch {
                    viewModel.mealsState.collect { state ->
                        when (state) {
                            ...
                            Resource.Loading -> {
                                mainActivity?.showLoader("Loading mess meals...")
                            }
                            is Resource.Success -> {
                                mainActivity?.hideLoader()
                                adapter.submitList(state.data)
                                binding.tvInfo.text = "You have ${state.data.size} items in menu"
                            }
                        }
                    }
                }
                    ...
                    viewModel.actionState.collect { state ->
                        when (state) {
                            ...
                            is Resource.Success -> {
                                ...
                                clearForm()
                                showForm(false)
                                Toast.makeText(requireContext(), "Menu updated", Toast.LENGTH_SHORT).show()
                            }
                            null -> Unit
                        }
                    }
